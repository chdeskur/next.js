...
export async function handler(
  ...
  if (!routeModule.isDev && !isDraftMode && hasStaticProps) {
    ...
    cacheKey = cacheKey === '/index' && pathname === '/' ? '/' : cacheKey
  }

  if (hasStaticPaths && !isDraftMode) {
    const isPrerendered = prerenderManifest.routes[decodePathParams(pathname)]
    ...
    const handleResponse = async (span?: Span) => {
      const responseGenerator: ResponseGenerator = async ({
          ...
          routeModule
            ...
            .then((renderResult): ResponseCacheEntry => {
              ...
              if (!hasStaticProps || isIsrFallback) {
                ...
                const notFoundRevalidate = getRequestMeta(
                  req,
                  'notFoundRevalidate'
                )
                ...
                // a number, use it.
                if (typeof metadata.cacheControl.revalidate === 'number') {
                  if (metadata.cacheControl.revalidate < 1) {
                    throw new Error(
                      `Invalid revalidate configuration provided: ${metadata.cacheControl.revalidate} < 1`
                    )
                  }

                  cacheControl = {
                    revalidate: metadata.cacheControl.revalidate,
                    expire:
                      metadata.cacheControl?.expire ?? nextConfig.expireTime,
                  }
                } else {
                  // revalidate: false
                  cacheControl = {
        ...
        if (
          ...
          res.statusCode = 404
          // on-demand revalidate always sets this state
      ...
      if (result.cacheControl && !res.getHeader('Cache-Control')) {
        res.setHeader(
          'Cache-Control',
          getCacheControlHeader(result.cacheControl)
        )
      }
      ...
      await sendRenderResult({
        ...
        cacheControl: routeModule.isDev ? undefined : result.cacheControl,
